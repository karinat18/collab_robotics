...
class CircleTracking(Node):
    ...
    def mobile_base_callback(self, data):
        ...
        """
        t = (self.get_clock().now().nanoseconds - self.t_init)*1e-9
        self.target_pose.position.x = self.r*np.cos(t*self.w)
        self.target_pose.position.y = self.r*np.sin(t*self.w)
    ...
    def go_to_pose(self, target_pose=None):
        ...
        """
        self.target_pose.position.x = self.r*np.cos(0.0)
        self.target_pose.position.y = self.r*np.sin(0.0)
        self.t_init = self.get_clock().now().nanoseconds
